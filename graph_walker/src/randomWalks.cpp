...
py::list randomWalksRestart(
    py::array_t<uint32_t> _indptr,
    py::array_t<uint32_t> _outdegrees,
    ...
    float alpha)
{
    ...
    py::buffer_info outdegBuf = _outdegrees.request();
    uint32_t *outdegrees = (uint32_t *)outdegBuf.ptr;
    ...
    PARALLEL_FOR_BEGIN(shape)
    {
        ...
        {
            uint32_t start = indptr[step];
            uint32_t end = start+outdegrees[step];
            ...
            {
                // probability alpha the walk stop
                break;
            }
            else
            {
        ...
        // convert walk vector to py::array and append it to the list
        std::lock_guard<std::mutex> lock(walksMutex);
        py::array_t<uint32_t> pyWalk(walk.size());
        std::memcpy(pyWalk.mutable_data(), walk.data(), walk.size() * sizeof(uint32_t));
        _walks.append(pyWalk);
    }
    PARALLEL_FOR_END();
